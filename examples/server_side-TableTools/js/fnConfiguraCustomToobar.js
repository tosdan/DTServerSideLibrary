...
function configuraCustomToobar(classeDivCustomToolbar, idTable, oTable, arrayTitoliColonne)
{
	...
	var classPulsante = 'btn' ,
		...
		idAzione = 'bAzione' ,
		bAzione = "<a href='javascript:void(0)' class='"+ classPulsante +"' id='"+ idAzione +"' ><i class='icon-ok'></i> Azione</input>" ,
	...
	$(classeDivCustomToolbar).html(	bSeleziona_tutto + " " 	
									+ bAnnulla_selezione + " " 
									+ bAzione + " " 
									+ bElimina); 
	...
	setActionPulsanteAzione(oTable, idAzione, classeDivCustomToolbar, arrayTitoliColonne, 'expand;nascosta');
	setActionPulsanteElimina(oTable, idElimina, classeDivCustomToolbar, 'expand;nascosta');
	
	dtFn.setExpandAllExtraEvent(oTable , idTable, dtFn.fnFormatDetails);
	dtFn.setExpandExtraEvent(oTable , idTable, dtFn.fnFormatDetails);
}
